...
- Enterprise Master Architecture & 50-Point Platform Charter (`docs/MASTER_ARCHITECTURE.md`):
  - General-Purpose Scope: SkyWatch is a general-purpose enterprise quality engineering, test management, automation, reporting, and agentic AI platform across Web, Mobile (iOS/Android), Desktop, APIs, Microservices, Databases, Cloud, Legacy, and SaaS.
  - Agentic Orchestration Principle: Never build hardcoded technology workflows (`if app == X`, `if framework == X`). Agents orchestrate dynamically via: Objective → Context → Planning → Capability Discovery → Tool Selection → Execution → Observation → Validation → Adaptation → Result.
  - Capability-Based Design (`backend/app/core/capabilities.py`): Pluggable `CapabilityRegistry` decoupling testing intent from specific libraries (`UI_BROWSER_AUTOMATION`, `API_AUTOMATION`, `MOBILE_AUTOMATION`, `DATABASE_VALIDATION`, etc.).
  - Standardized Tool Registry (`backend/app/core/tool_registry.py`): Every tool implements standard metadata (ID, Name, Description, Capability, Version, Input/Output Schema, Permissions, Supported Environments, Timeouts, Retries, Availability).
