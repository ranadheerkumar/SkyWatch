...
const PREPARED_EXECUTION_CASES_STORAGE_KEY = "ai-qa-engine:prepared-execution-cases";
const LATEST_GENERATED_CASES_STORAGE_KEY = "ai-qa-engine:latest-generated-cases";
const EXECUTION_CONFIGURATION_STORAGE_KEY = "ai-qa-engine:execution-configuration";
...
function createRuntimeParameter(): RuntimeParameter {
  return {
...
function normalizeLoginSelector(selector: string): string {
  return selector.trim().replace(/\s+/g, " ").toLowerCase();
}

function validateLoginSelectors(emailSelector: string, passwordSelector: string, submitSelector: string): string | null {
...
function validateRuntimeParameters(parameters: RuntimeParameter[], requireLoginCredentials = false, loginSelectors: string[] = []): string | null {
...
function buildRuntimeParameterMap(parameters: RuntimeParameter[]): Record<string, string> {
...
function isSensitiveRuntimeParameter(parameter: RuntimeParameter): boolean {
...
function buildGenerationParameterContext(parameters: RuntimeParameter[]): string {
...
function readPreparedExecutionCaseIds(): number[] {
...
function readLatestGeneratedCaseIds(): number[] {
  if (typeof window === "undefined") return [];
  try {
...
const sortApplicationsForDisplay = (items: Application[]) => [...items].sort((left, right) => {
  return left.name.localeCompare(right.name);
});
const mapApiApplication = (item: { id: number; name: string; platform: string; target: string }, fallbackCases = 0): Application => ({
  id: item.id,
  name: item.name,
...
async function loadAllTestCases(authToken: string): Promise<TestCase[]> {
  ...
  return (
    <>
      <div className="page-heading">
        ...
        {action && !action.includes("Add new application") ? <button className="primary btn-primary" onClick={() => onAction?.()} disabled={actionDisabled}>{action}</button> : null}
      </div>
      {action?.includes("Add new application") && onAction ? <ApplicationCapture onSaved={(savedApplicationName) => onAction(savedApplicationName)} /> : null}
    </>
  );
}
...
function TableScroll({ children, containedScroll }: { children: React.ReactNode; containedScroll: boolean }) {
...
function buildCategoryCoverage(cases: TestCase[]) {
...
function buildExecutionTrend(runs: RunSummary[], days = 7) {
...
function buildDefectTrend(defects: Defect[], points = 7) {
...
function buildApplicationCaseDistribution(applications: Application[], cases: TestCase[]) {
...
function buildApplicationPassRateDistribution(applications: Application[], runs: RunSummary[]) {
  return applications.map((application) => {
...
function buildDefectPriorityDistribution(defects: Defect[]): DistributionItem[] {
...
function buildDefectStatusDistribution(defects: Defect[]): DistributionItem[] {
  let open = 0;
  let inProgress = 0;
  let resolved = 0;
  defects.forEach((defect) => {
...
function buildRunStatusDistribution(runs: RunSummary[]): DistributionItem[] {
...
function ExecutionArtifactPreview({
  artifacts,
  token,
  runId,
  showHeading = true,
...
function ExecutionStepEvidence({
  steps,
  artifacts,
  token,
  runId,
...
function formatProjectLifecycle(lifecycle: ProjectLifecycle) {
...
function inferEnvironmentLabel(target?: string) {
...
function inferProjectHealth(passRate: number, openDefectCount: number, completedRuns: number): { bucket: ProjectHealthBucket; label: string } {
  if (completedRuns === 0) {
...
function normaliseStatusValue(status?: string) {
  return (status ?? "").trim().toLowerCase().replace(/_/g, " ");
}

function formatFailureTypeLabel(failureType?: FailureType | string | null) {
...
const LIVE_EVENT_PREFIX = "LIVE_EVENT|";
const VOICE_MODE_STORAGE_KEY = "ai-qa-engine:voice-mode";
const VOICE_GENDER_HINTS: Record<VoiceGender, RegExp> = {
...
function findSpeechVoice(gender: VoiceGender) {
...
function parseLiveEventsFromLog(log?: string): Array<{ state: string; message: string }> {
  if (!log) return [];
...
function latestLiveEvent(log?: string): { state: string; message: string } | null {
  const events = parseLiveEventsFromLog(log);
  return events.length ? events[events.length - 1] : null;
}

function mapLiveStateToExecutionPhase(state?: string): "idle" | "queueing" | "running" | "finalizing" | "done" | "error" {
...
function formatLiveStateLabel(state?: string) {
  const normalized = (state ?? "").trim().toUpperCase();
  if (!normalized) return "RUNNING";
  return normalized
    .split("_")
...
function resolveExecutionSpeechRate(slowMode: ExecutionSlowMode = "normal"): number {
  if (slowMode === "showcase") return 0.92;
  if (slowMode === "demo") return 0.98;
  return 1.05;
}

function pumpExecutionSpeechQueue() {
...
function stopExecutionSpeech() {
  executionSpeechQueue = [];
  executionSpeechActive = false;
  if (typeof window !== "undefined" && "speechSynthesis" in window) window.speechSynthesis.cancel();
}

function speakExecutionMessage(message: string, rate = 1, voiceGender: VoiceGender = "male", queue = false) {
  ...
  if (!queue) {
    ...
    // If a new step action arrives, prune any pending unstarted steps from the queue to maintain lockstep
...
function isExecutionSpeechAction(message: string) {
  return /^(?:step\s+\d+(?:\s*(?:\/|of)\s*\d+)?\s*:\s*|opening\b|navigating\b|application loaded\b)/i.test(message);
}

function cleanExecutionSpeech(message: string) {
  return message
...
function isVerificationOnlyLine(lowerLine: string) {
  return /^(verify|validate|confirm|check)\b/.test(lowerLine);
}

function isInvalidCredentialInstruction(lowerLine: string) {
...
function isLoginInstructionLine(lowerLine: string) {
  if (isVerificationOnlyLine(lowerLine)) return false;
...
function normalizeActionTargetText(value: string) {
...
function extractVerificationText(line: string, quoted: string[]) {
...
function buildPollingPlan(stepCount: number, checkCount: number, mode: ExecutionMode) {
...
function getStatusMeta(status?: string): { label: string; icon: string; tone: "success" | "info" | "warning" | "danger" | "neutral" } {
  const normalized = normaliseStatusValue(status);
...
function formatConfidenceLabel(label: "high" | "medium" | "low") {
  return label.charAt(0).toUpperCase() + label.slice(1);
}

function renderWorkflowStatusChip(state: "completed" | "inProgress" | "queued" | "attention") {
...
function formatConfidenceChipLabel(score: number, label: "high" | "medium" | "low") {
...
function normaliseCaseKey(value?: string) {
  return (value ?? "").trim().toLowerCase().replace(/\s+/g, " ");
}

function parseAiGenerationSourceFromJob(job: AIGenerationJob): AIGenerationSource {
...
function normalizeGeneratedCases(generatedCases: TestCase[], applicationId: number) {
...
function parseStepEntries(steps?: string): Array<{ number: string; text: string }> {
...
function inferRunScopeKey(status?: string) {
...
function buildCaseTableRows(cases: TestCase[]): CaseTableDisplayRow[] {
